...
static const char *const CF_HASH_DIGEST_TYPES[10] =
{
    "md5",
    "sha224",
    "sha256",
...
static const int CF_HASH_DIGEST_SIZES[10] =
{
    CF_MD5_LEN,
    CF_SHA224_LEN,
    CF_SHA256_LEN,
...
Hash *HashBasicInit(HashMethod method)
{
    ...
    hash->size = CF_HASH_DIGEST_SIZES[method];
    hash->method = method;
    return hash;
}
...
Hash *HashNew(const char *data, const unsigned int length, HashMethod method)
{
    ...
    int md_len = 0;
    md = EVP_get_digestbyname(CF_HASH_DIGEST_TYPES[method]);
    if (md == NULL)
    {
        return NULL;
    }
...
{
    int i;
    for (i = 0; CF_HASH_DIGEST_TYPES[i] != NULL; i++)
    {
        if (hash_name && (strcmp(hash_name, CF_HASH_DIGEST_TYPES[i]) == 0))
        {
            return (HashMethod) i;
        }
    }
...
{
    return (hash_id >= HASH_METHOD_NONE) ? NULL : CF_HASH_DIGEST_TYPES[hash_id];
}

HashSize HashSizeFromId(HashMethod hash_id)
{
    return (hash_id >= HASH_METHOD_NONE) ? CF_NO_HASH : CF_HASH_DIGEST_SIZES[hash_id];
}
